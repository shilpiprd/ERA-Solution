...
from utils import progress_bar, visualize_misclassified_images
...
# ])
transform_train = A.Compose([ #modified this slightly
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            misclassified_idxs = (predicted != targets).nonzero(as_tuple=False).squeeze()
            for idx in misclassified_idxs:
                if len(misclassified_images) < 20:  # Collect only 20 images
                    img = inputs[idx].cpu()          #data is inputs
                    actual_label = targets[idx].item()
...
for epoch in range(start_epoch, start_epoch+20):
    train(epoch)
    misclassified = test(epoch)
    scheduler.step()


#use a function from utils to show these misclassified images 
print('giving out misclassified Images:')
